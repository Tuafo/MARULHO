...
→ 614 passed, 7 subtests passed (across 54 test files)
```
...
- Test suite: 614 passed, 7 subtests passed.
...
*Full 5-stage developmental protocol validated with multimodal training throughout all stages (not just stages 1–2). Null-control validation: untrained models fail stages 3–5 (probe=0.34, gap=−0.28). Trained results: probe 0.64–0.68 across seeds. Audio self-criticism wired alongside visual. Text-only HECSN control: 0.42 total, gap −0.24 (confirms multimodal is responsible for concrete advantage). 612 tests pass across 54 test files. GPU routing: sub-1ms at 100K columns (0.67ms median, RTX 3060). Scale test: 100K steps at 31.2 steps/s, 10/10 digits grounded (0.86–0.91). Architecture activation: all 7 paper-described layers enabled progressively via _make_config_for_stage() (614 tests pass).*
...
5. **Test count synchronized.** 614 passed, 7 subtests passed across 54 test files.
...
3. **1M-token scale test launched (in progress).** GPU (RTX 3060), 256 columns, wikitext-103 corpus (15M chars). At 600K tokens: 46 tok/s sustained, 10MB GPU memory, no degradation. Stage 1 only (uses HECSNTrainer directly). Expected completion: ~6 hours total.
