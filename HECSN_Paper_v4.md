...
**Version:** 4.11 — Audited, Implementation-Current, Scale-Validated Architecture Document
...
Five-stage developmental protocol runs end-to-end with state continuity (ProtocolState, including concept_signatures). All 5 stages pass across tested seeds (42, 7, 123) at 5,000 tokens per stage. **All stages use multimodal training** — concept-conditioned visual/audio spikes are paired throughout stages 1–5, not just during initial grounding (stages 1–2). Audio self-criticism is wired alongside visual self-criticism. Stage 3–5 criteria include absolute grounding thresholds (probe ≥ 0.52, concreteness_gap > 0.0) validated via null-control test: untrained models fail all stages (probe=0.34, gap=−0.28). Key mechanism: per-word sensory signatures (cell assembly encoding) with lateral inhibition (centering) and routing-key fading (rk_weight = 1 − word_conf) provide the discriminative power for grounding probe accuracy 0.64–0.68, well above baselines (fastText 0.46, SOM 0.46). **Scale validation (v4.11):** 50,000 steps completed with real N-MNIST + FSDD data at 39.0 steps/s. All 10 digit words grounded (confidence 0.85–0.91). Remaining: 100K+ scale, GPU acceleration.

### Phase 8: Paper ⬜ IN PROGRESS

This paper (HECSN_Paper_v4.md, v4.11) is the current publication draft. Architecture complete, 5-stage developmental protocol validated with 50-triple probe results exceeding baselines, results table fully populated (zero "measure" cells). Training performance validated at 39 steps/sec real multimodal (N-MNIST + FSDD, 50K steps sustained) and 97 steps/sec synthetic multimodal. **50K-step scale test completed:** 50,000 steps in 1281s (39.0 steps/s) with real N-MNIST visual events + FSDD spoken audio. All 10 digit words grounded to confidence 0.85–0.91 after 1000 episodes. Remaining work: 100K+ scale validation, GPU acceleration, 8–10 page submission format.
...
→ 608 passed, 7 subtests passed (across 54 test files)
```
