"""Extended stability test for Terminus — 30min multimodal run.

Tracks: tick rate, throughput, memory fill, multimodal episodes,
neuromodulators, concept count, errors.
"""
...
print(f"=== Terminus Extended Stability Test ({RUN_SECONDS}s) ===")
print(f"Start: {datetime.now().isoformat()}")

# Quick-start multimodal
r = requests.post(f"{BASE}/terminus/quick-start?preset=multimodal", timeout=30)
...
try:
    while (time.monotonic() - start_time) < RUN_SECONDS:
        ...
        try:
            ...
            mm = ts.get("multimodal") or {}

            # Full status for neuromod + memory
            ...
            point = {
                ...
                "n_concepts": st.get("concept_store", {}).get("num_concepts", 0),
            ...
            status_line = (
                ...
                f"mm_eps={point['mm_eps']:>3d} "
                f"fill={point['fill_frac']:.2f} "
                f"concepts={point['n_concepts']:>4d} "
