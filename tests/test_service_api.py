...
class ServiceApiTerminusRuntimeTests(unittest.TestCase):
    ...
    def test_snn_language_readout_draft_endpoint_generates_bounded_grounded_text(self) -> None:
        ...
        ledger_normalization_window = ledger["summary"]["snapshot_source_window"]
        self.assertEqual(
        ...
        self.assertEqual(
            ledger["summary"]["normalization_source_window"],
            ledger_normalization_window,
        )
        self.assertEqual(
    ...
    def test_terminus_configure_and_tick_endpoint_train_from_file_source(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            self.assertNotIn("replay_dataset_summary", status_response.json())
            self.assertNotIn(
                "replay_dataset_summary",
                status_response.json()["terminus_runtime"]["living_loop"],
            )
    ...
    def test_retired_service_replay_endpoints_return_404(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            root = Path(tmpdir)
            app = create_app(
                _build_checkpoint(root, test_case="service_api_retired_replay_surfaces"),
                trace_dir=root / "traces",
                env_root=root,
            )
            with TestClient(app) as client:
                responses = {
                    "replay_plan": client.get("/terminus/replay-plan?limit=5"),
                    "replay_sample": client.post(
                    ...
                    "replay_sample_history": client.get("/terminus/replay-sample/history?limit=5"),
                    "replay_dataset_preview": client.get("/terminus/replay-dataset/preview?limit=5"),
                    "replay_dataset_bundle": client.post(
                        "/terminus/replay-dataset/bundle",
                        json={"operator_id": "operator-a", "confirmation": True},
                    ),
                    "replay_dataset_candidates": client.get("/terminus/replay-dataset/candidates?limit=5"),
                    "replay_dataset_history": client.get("/terminus/replay-dataset/history?limit=5"),
                }
                status_response = client.get("/status")
                living_response = client.get("/terminus/living-loop")
        ...
        self.assertNotIn("replay_dataset_summary", status_body)
        self.assertNotIn("replay_plan", living_loop)
        self.assertNotIn("replay_sample_summary", living_loop)
        self.assertNotIn("replay_dataset_summary", living_loop)
        self.assertNotIn("replay_plan_summary", export_body)
        self.assertNotIn("replay_sample_summary", export_body)
        self.assertNotIn("replay_dataset_summary", export_body)
    ...
    def test_feed_query_respond_endpoints_return_runtime_episodes(self) -> None:
        ...
        for example in export_body["examples"]:
            ...
            self.assertNotIn("replay_plan_summary", example)
