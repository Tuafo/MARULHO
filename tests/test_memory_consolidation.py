...
class MemoryConsolidationTests(unittest.TestCase):
    ...
    def test_recent_anchor_capture_uses_capped_recency_index(self) -> None:
        ...
        self.assertEqual(report["anchor_row_surface"], "bounded_recent_anchor_capture_row.v1")
        self.assertTrue(report["anchor_row_reader_owned_by_store"])
        self.assertEqual(report["anchor_row_read_count"], 3)
        self.assertEqual(report["invalid_anchor_row_count"], 0)
        self.assertTrue(report["direct_slow_memory_bucket_reads_retired"])
        self.assertFalse(report["raw_text_payload_loaded"])
        self.assertFalse(report["language_reasoning"])
        self.assertFalse(report["runs_every_token"])
        self.assertFalse(report["global_score_scan"])
