...
- Exact torch-cache routing may join that selector only through the promoted graph text-tick island. The old `fused_triton_text` checkpoint/config selector is retired; retained tensor and fused-only route/vote are internal fallback/evaluation controls rather than production runtime choices. Retrieval exposes the current cache without copying, core owns the two Triton kernels, and `ColumnTransitionRuntime` owns compile-only warmup, persistent workspaces, cache-pointer refresh, execution/fallback counters, and fail-closed selection state.
...
- The fixed promoted `predictive_route_vote_mode=cuda_graph_text` path captures a fixed-address text-tick island containing production input normalization/projection, exact fresh reconstruction distance, fused route/vote, and the in-place transition. It remains text/idle-only and must bypass graph pre-routing before any visual/audio tick.
...
- Runtime Truth must expose graph capture success/latency/count, fixed-address status, pre-route replay and sensory-bypass counts, transition replay/failure counts, graph names, tensor device, configured route-vote mode, and requested-mode source. The default and only production route-vote config is `cuda_graph_text`; rollback uses internal fallback/evaluation controls rather than restoring retired public selectors.
