...
---
        ...
        Standalone compiled route/competition remains rejected, but the route-candidate-bank plus predictive-vote cluster is now the default eligible CUDA text path through fixed `predictive_route_vote_mode=cuda_graph_text`. The former `tensor` and `fused_triton_text` selectors are retired from production config and checkpoint restore; they survive only as explicit evaluation/fallback controls with `route_vote_requested_mode_source` evidence. The exact torch cache remains the seed/oracle, while steady graph/burst ticks score the training-owned route bank plus a fixed two-row probe lane through `route_positions`. The fused route/vote select kernel now writes the next route-bank and probe positions on device, so `ColumnTransitionRuntime` owns compile-only warmup, persistent score/candidate state, device refresh counters, seed/restore refresh fallback, execution evidence, and sensory fallback. It runs only on text/idle ticks; visual/audio ticks retain ordinary tensor routing.
        ...
        The maintained steady route/vote kernel variant is `indexed_route_bank_vote_device_refresh`: a Triton route-score kernel over the bounded bank followed by a select/vote kernel that also writes the next bank/probe positions. The old `two_stage_route_vote` remains a truthfully reported exact-cache seed/oracle or fallback shape, and `indexed_route_bank_vote` without device refresh remains only a direct evaluation/control shape, not the steady scaling boundary. A direct one-block route/vote fusion for the live `1024 x 64` checkpoint passed parity but regressed complete runtime, so it was deleted instead of retained as dormant hot-path code. Runtime Truth keeps the variant field so future speed work can prove whether a real executor is running the retained route/vote family or a newly promoted replacement.
        ...
        Slow replay-memory admission is no longer a fixed-cadence or every-strong hot-path write. Every token still runs the promoted column transition, context, binding, cross-modal, surprise, and routing-index buffer policies, but expensive `DualMemoryStore.update()` admission and stream-text episode reconstruction run only on retained/fallback admission or selected high-surprise strong-capture events. Fixed cadence is counted as deferred maintenance by the cognitive boundary controller, not as a reason to break burst execution. Strong-capture admission has its own refractory cadence through `slow_memory_archive_strong_capture_min_interval_tokens` and rejects production values `<=1`, so a low capture threshold cannot archive every token. Runtime Truth exposes deferred cadence, archive count, skip count, interval, last archive reason, strong-capture min interval, strong archive count, refractory skip count, and last strong-capture archive token through `memory_hot_path` and the boundary report.
