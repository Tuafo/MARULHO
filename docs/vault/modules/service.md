...
---
        ...
        Public SNN language/readout route payloads should use readout vocabulary. The readout-surface and readout-memory chains are canonical end-to-end through service, facade, ledger, and checkpoint save state; legacy `autonomous_snn_language_thought_surface_*` and `autonomous_snn_language_thought_memory_*` persisted fields migrate once and are not active API/facade aliases. The remaining readout route boundary still translates older downstream consolidation, structural-plasticity, capacity-mutation, newborn-neuron, critical-period, maturation, and newborn-synapse-pruning ledger shapes for operators and UI clients. It is a compatibility bridge for not-yet-renamed downstream families, not a license to add new thought-era public surfaces.
