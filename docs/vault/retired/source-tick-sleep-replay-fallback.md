...
The maintained path passes `allow_sleep_maintenance=False` from source-tick fallback. Due sleep is recorded as deferred trainer maintenance, and explicit trainer sleep/replay windows remain the only execution path for replay computation. The same gate is now threaded through delegated `train_text_sequence(...)`; if training-owned sequence execution falls back to per-token `train_step(...)`, the fallback forwards `allow_sleep_maintenance=False` and reports fallback/deferred counts instead of running sleep replay. Archival storage stays CPU-resident; this retirement does not move metadata to CUDA, scan full memory, or use replay text as hidden language reasoning.
...
The refreshed deferral benchmark `reports/bounded_replay_window_20260622/source-tick-sequence-sleep-deferral.json` covers both the original service fallback and the delegated sequence fallback. It passed with service fallback sleep calls `0`, sequence fallback sleep calls `0`, explicit slow-path sleep calls `1`, and visible sequence fallback deferred counts. The paired `524288`-token protection run `reports/bounded_replay_window_20260622/hotpath-active-pressure-65536-524288-i32-sleep-replay-associative-recall-source-sequence-deferral.json` stayed in band at `6487.329 tokens/sec`, `train_compute=0.125633 ms/token`, bounded route scoring at `12/65536`, cached `65526` transition rows, no observed contention, RTX 3060 memory `1709->1707 MiB`, and zero graph/native sequence failures.
