...
summaries instead of preserving them as bounded side paths. The hot-path claim
to verify is therefore absence: no service replay planning, sample history,
dataset preview, dataset bundle, status summary, trace-export summary, or
benchmark endpoint may enter the live tick. Runtime trace export remains an
explicit slow tooling surface, and ReplayController keeps only SNN
artifact/permit/ticket review windows. The paired hot-path run for this slice
...
The active benchmark now reports only `baseline_quantum_*`, `candidate_quantum_*`, and `candidate_over_baseline_quantum_speedup` fields, and runs only reversed-order maintained baseline/candidate quantum arms. The historical one-token/yield arms and compatibility report keys are retired-path evidence only. The cleanup test run passed with `42` focused tests and the paired `524288`-token gate reached `5561.260 tokens/sec`, p95 tick `24.217 ms`, `train_compute=0.143169 ms/token`, bounded `12/65536` route rows, no observed contention, RTX memory `1682->1695 MiB`, and zero native sequence-loop or burst-replay failures. A future quantum-size change must beat the current service-shaped path, not only a narrower encoded loop. The next credible thousands/sec path is a broader persistent tick executor that removes more host-controlled per-token stages; increasing host quantum length alone is rejected.
...
### Merged Source-Bank Memory Match, 2026-06-22

The source-bank memory-match slice now makes source acquisition recall
auditable at the bank-planning layer with one merged local associative window.
`bank_memory_matches_with_report(...)` samples capped source-bank probes,
...
against that window, and loads raw text only for returned matches. The report
records `bounded_source_bank_memory_match.v1` with
...
`language_reasoning=false`. The paired benchmark
...
reduced raw text payload loads from `32` to `4`, and reduced mean latency from
`560.177 ms` to `106.543 ms` (`5.258x`) over a `65536`-entry archive. CUDA was
available but archival recall allocated/reserved `0.0 MiB`.

The clean 65536-column 524288-token protection run was:
...
It processed `524288` tokens at `6129.933 tokens/sec`, with
`train_compute=0.132126 ms/token`, `prepare_training=0.006695 ms/token`,
`finalize_total=0.006421 ms/token`, and `tick_duration_ms.p95=20.698`.
Runtime Truth stayed bounded at `route_input_rows_scored=12/65536`,
`route_output_candidate_count=10`, `state_transition_cached_count=65526`, and
`state_transition_runs_all_columns=false`. Graph, native sequence, and native
burst failures were all `0`, with `32767` conditional sequence-loop successes
covering `524272` tokens. The velocity surface reported mild GPU contention:
CPU max `41%`, GPU utilization max `21%` against a `20%` threshold, GPU memory
utilization max `22%`, and GPU memory stayed flat at `1763 MiB`. This is
...
This slice removes the remaining missing-key fallback from selected repair
replay. The previous repair path used stored routing keys when present, but
entries without stored routing keys could rebuild a routing key from the input
pattern or project the already-selected stored assembly trace. The maintained
path requires a stored routing key; missing-key entries are deferred and reported
as `sleep_replay_missing_routing_key_deferred_count`. The old
`sleep_replay_stored_assembly_projection_fallback_count` field is no longer
emitted, and `sleep_replay_dense_input_assembly_fallback_count=0` stays true.
...
`python -m marulho.evaluation.continuous_runtime_stress_benchmark --checkpoint reports\column_scheduler_20260618\checkpoints\active-pressure-scheduler-65536-seeded.pt --output reports\bounded_replay_window_20260620\hotpath-active-pressure-65536-524288-i32-repair-missing-routing-key-deferred.json --target-tokens 524288 --tick-tokens 128 --quantum-tokens 16 --source-concept-observation-tick-interval 4 --timeout-seconds 900 --sample-interval-seconds 0.05 --host-truth-sync-interval-tokens 32`

It processed `524288` tokens at `5988.223 tokens/sec`, with
`train_compute=0.135762 ms/token`, `prepare_training=0.006894 ms/token`,
`finalize_total=0.006597 ms/token`, and `tick_duration_ms.p95=21.650`.
Runtime Truth stayed bounded at `route_input_rows_scored=12/65536`,
`route_output_candidate_count=10`, `state_transition_cached_count=65526`, and
`state_transition_runs_all_columns=false`. Graph, selection, native sequence,
and native burst failures were all `0`. The velocity surface reported CPU max
`17%`, GPU utilization max `31%`, GPU memory utilization max `23%`, and GPU
memory moved from `1877 MiB` to `1878 MiB`; because the GPU sample crossed the
contention threshold, this is protection evidence rather than a speed ceiling.
...
After the 2026-06-23 cleanup removed the benchmark-local full-retained
emission-review and status replay comparators, the current worktree reran the
default no-sampling 65536-column gate:

`python -m marulho.evaluation.continuous_runtime_stress_benchmark --checkpoint reports\column_scheduler_20260618\checkpoints\active-pressure-scheduler-65536-seeded.pt --output ..\..\MARULHO_reports\bounded_replay_window_20260623\hotpath-active-pressure-65536-524288-i32-emission-status-legacy-baselines-removed-default-nosample.json --target-tokens 524288 --tick-tokens 128 --quantum-tokens 16 --source-concept-observation-tick-interval 4 --timeout-seconds 900 --sample-interval-seconds 0.05 --environment-sample-interval-seconds 0 --host-truth-sync-interval-tokens 32 --profile-trainer-stages`

It processed `524288` tokens in `80.430409 s` at
`6518.530 tokens/sec`, with `tick_duration_ms.p95=19.4218`,
`train_compute=0.124903 ms/token`, `prepare_training=0.005812 ms/token`, and
`finalize_total=0.005858 ms/token`. Runtime Truth stayed bounded at
`route_input_rows_scored=12/65536`, `route_output_candidate_count=10`,
`state_transition_cached_count=65526`, and
`state_transition_runs_all_columns=false`; graph, native burst, and native
sequence failures were all `0`. Device evidence selected CUDA on the RTX 3060,
kept measured-window environment polling disabled (`sample_count=0`), reported
no observed contention, and kept GPU memory flat at `1747 MiB` before/after the
run. Prewarm took `246542.149 ms`, outside the measured hot path.
...
The known-readout source-window artifact-binding protection rerun was:
...
`finalize_total=0.006545 ms/token`, and `tick_duration_ms.p95=22.165`.
Runtime Truth stayed bounded at `route_input_rows_scored=12/65536`,
`route_output_candidate_count=10`, `state_transition_cached_count=65526`, and
`state_transition_runs_all_columns=false`. Graph, native burst, and native
sequence failures were all `0`; conditional-WHILE q16 remained active. The
velocity sampler reported GPU contention, CPU max `49%`, GPU max `30%`, GPU
memory-util max `23%`, and RTX 3060 memory stayed flat at `1986 MiB`. The first
same-code run reached `5728.814 tokens/sec` under heavier GPU contention, so
...
The raw replay-artifact recorder retirement protection run was:
...
`278.046 s`. Runtime Truth stayed bounded at
`route_input_rows_scored=12/65536`, `route_output_candidate_count=10`,
`state_transition_cached_count=65526`, and
`state_transition_runs_all_columns=false`. Graph, native burst, and native
sequence failures were all `0`; conditional-WHILE q16 remained active. The
velocity sampler observed contention, CPU max `32%`, GPU max `28%`, GPU
...
The source tick sleep replay deferral protection run was:
...
`finalize_total=0.006522 ms/token`, `tick_duration_ms.p95=21.728`, and prewarm
`267.046 s`. Runtime Truth stayed bounded at
`route_input_rows_scored=12/65536`, `route_output_candidate_count=10`,
`state_transition_cached_count=65526`, and
`state_transition_runs_all_columns=false`. Graph, native burst, and native
sequence failures were all `0`; conditional-WHILE q16 remained active. The
velocity sampler reported no observed contention, CPU max `56%`, GPU max
...
and readout-structural-plasticity now use the same record-family source-window
...
`python -m marulho.evaluation.snn_readout_ledger_normalization_source_window_benchmark --retention-count 2048 --ledger-limit 128 --runs 3 --output reports\bounded_replay_window_20260622\snn-readout-ledger-normalization-readout-memory-canonical.json`

Result: `pass=true`; after the production memory-chain rename, the bounded
path preserved canonical readout-memory hash/review/count behavior. Bounded
mean was `380.146067 ms` versus legacy diagnostic `5619.687233 ms` (`16x`
source-work reduction), and the autonomous-chain bounded mean stayed
`944.358700 ms` versus `19539.615767 ms`. Archival/source/review placement is
CPU-only; CUDA was available on the RTX 3060 but unused for archival ledger
metadata, with `0.0 MiB` CUDA allocation/reservation and Python tracemalloc peak
`429.774527 MiB`. No live tick, every-token cadence, replay execution, or
hidden language reasoning is involved.

Accepted hot-path protection rerun:

`python -m marulho.evaluation.continuous_runtime_stress_benchmark --checkpoint reports\column_scheduler_20260618\checkpoints\active-pressure-scheduler-65536-seeded.pt --output reports\bounded_replay_window_20260622\hotpath-active-pressure-65536-524288-i32-readout-memory-canonical-rerun.json --target-tokens 524288 --tick-tokens 128 --quantum-tokens 16 --source-concept-observation-tick-interval 4 --timeout-seconds 900 --sample-interval-seconds 0.05 --host-truth-sync-interval-tokens 32 --profile-trainer-stages`

Result: `success=true`, `524288` tokens in `87.568988 s`,
`5987.142 tokens/sec`, tick p95 `21.546300 ms`, `train_compute=0.134307
ms/token`, `prepare_training=0.007037 ms/token`, and
`finalize_total=0.006893 ms/token`. Prewarm was explicit slow-path work
(`324830.668 ms`) and reached `full_warm_ready=true` before measurement. Route
...
Readout-structural-plasticity canonicalization benchmark:

`python -m marulho.evaluation.snn_readout_ledger_normalization_source_window_benchmark --retention-count 2048 --ledger-limit 128 --runs 3 --output reports\bounded_replay_window_20260622\snn-readout-ledger-normalization-readout-structural-canonical.json`

Result: `pass=true`; after retiring the
`autonomous_snn_language_thought_structural_plasticity_*` production names, the
bounded path preserved canonical readout-structural hash/review/count behavior.
Bounded mean was `568.337767 ms` versus legacy diagnostic `7518.428000 ms`
(`16x` source-work reduction), and the downstream autonomous-chain bounded mean
stayed `967.423500 ms` versus `21131.022800 ms`. Archival/source/review
placement is CPU-only; CUDA was available on the RTX 3060 but unused for
archival ledger metadata. No live tick, every-token cadence, replay execution,
raw replay text, or hidden language reasoning is involved.

Readout-structural-plasticity hot-path protection run:

`python -m marulho.evaluation.continuous_runtime_stress_benchmark --checkpoint reports\column_scheduler_20260618\checkpoints\active-pressure-scheduler-65536-seeded.pt --output reports\bounded_replay_window_20260622\hotpath-active-pressure-65536-524288-i32-readout-structural-canonical.json --target-tokens 524288 --tick-tokens 128 --quantum-tokens 16 --source-concept-observation-tick-interval 4 --timeout-seconds 900 --sample-interval-seconds 0.05 --host-truth-sync-interval-tokens 32 --profile-trainer-stages`

Result: `success=true`, `524288` measured tokens in `89.080214 s`,
`5885.572 tokens/sec`, tick p95 `22.879800 ms`, `train_compute=0.136747
ms/token`, `prepare_training=0.007215 ms/token`, and
`finalize_total=0.006836 ms/token`. Prewarm was explicit slow-path setup
(`418781.090 ms`) and reached `full_warm_ready=true` before measurement, so it
is not included in the steady throughput score. Route scoring stayed bounded at
`12/65536` input rows with `10` output candidates, `65526` cached transition
rows, `state_transition_runs_all_columns=false`, and zero graph/native sequence
failures. Runtime device was CUDA on an RTX 3060; the environment sampler
reported no observed contention, CPU max `88%`, GPU max `13%`, GPU memory util
max `18%`, and GPU memory `1741->1970 MiB`.

Historical full-chain quality benchmark:

`python -m marulho.evaluation.snn_readout_ledger_normalization_source_window_benchmark --retention-count 2048 --ledger-limit 128 --runs 3 --output reports\bounded_replay_window_20260619\snn-readout-ledger-normalization-thought-structural-chain.json`

Result: `pass=true`; the bounded chain preserved hash, review-match,
total-count, and current-pointer parity across the seventeen-component
autonomous readout/language/thought chain. Checked source rows fell from
`100096` to `4352` (`23x`), and mean chain latency fell from
`19704.406867 ms` to `1046.241300 ms` (`18.833520x`). CUDA was available on the
RTX 3060 but the benchmark used no GPU execution for archival ledger metadata;
archival/source/review placement stayed CPU-only with `runs_live_tick=false`,
`runs_every_token=false`, and `language_reasoning=false`.

Long protection rerun:

`python -m marulho.evaluation.continuous_runtime_stress_benchmark --checkpoint reports\column_scheduler_20260618\checkpoints\active-pressure-scheduler-65536-seeded.pt --output reports\bounded_replay_window_20260619\hotpath-active-pressure-65536-524288-i32-thought-structural-chain-rerun.json --target-tokens 524288 --tick-tokens 128 --quantum-tokens 32 --source-concept-observation-tick-interval 4 --timeout-seconds 300 --sample-interval-seconds 0.02`

Result: `success=true`, `524288` tokens in `87.305243 s`,
`6005.229 tokens/sec`, `tick_duration_ms.p95=22.012`,
`train_compute=0.135094 ms/token`, `prepare_training=0.007082 ms/token`, and
`finalize_total=0.006415 ms/token`. Runtime Truth kept route scoring bounded at
...
max=64%`, `gpu max=3%`, `gpu memory util max=1%`). Runtime CUDA memory moved
`1856->1857 MiB`.

The first same-shape long run at
`reports\bounded_replay_window_20260619\hotpath-active-pressure-65536-524288-i32-thought-structural-chain.json`
...
Synapse provenance audit no longer normalizes every retained readout-ledger
event family before validating readout-derived sparse-transition lineage. The
production path gathers requested hashes from `synapse_provenance_by_key` and
looks up only the `events` source window through
`bounded_snn_readout_evidence_event_map_source_window.v1`.

Focused quality benchmark:

`python -m marulho.evaluation.snn_readout_ledger_normalization_source_window_benchmark --retention-count 2048 --ledger-limit 128 --runs 3 --output reports\bounded_replay_window_20260619\snn-readout-ledger-normalization-synapse-provenance-map.json`

Result: `pass=true`; requested event-map hash parity was preserved. Checked
source rows fell from `2944` broad-normalized rows to `128` bounded `events`
rows (`23x`), and mean event-map latency fell from `319.823233 ms` to
`13.972533 ms` (`22.889424x`). CUDA was available on the RTX 3060 but the
benchmark used no GPU execution for archival ledger metadata; archival/lookup
placement stayed CPU-only with `runs_live_tick=false`, `runs_every_token=false`,
and `language_reasoning=false`.

Long protection run:

`python -m marulho.evaluation.continuous_runtime_stress_benchmark --checkpoint reports\column_scheduler_20260618\checkpoints\active-pressure-scheduler-65536-seeded.pt --output reports\bounded_replay_window_20260619\hotpath-active-pressure-65536-524288-i32-synapse-provenance-map.json --target-tokens 524288 --tick-tokens 128 --quantum-tokens 32 --source-concept-observation-tick-interval 4 --timeout-seconds 300 --sample-interval-seconds 0.02`

Result: `success=true`, `524288` tokens in `87.467178 s`,
`5994.111 tokens/sec`, `tick_duration_ms.p95=21.885`,
`train_compute=0.135406 ms/token`, `prepare_training=0.007135 ms/token`, and
`finalize_total=0.006412 ms/token`. Runtime Truth kept route scoring bounded at
`12/65536` input rows and `10` output candidates, with `65526` cached transition
rows, `state_transition_runs_all_columns=false`, and zero graph/native sequence
failures. The environment sampler reported no observed contention (`cpu
...
## Applied Synapse Provenance Audit Source Window

The exact readout synapse provenance audit no longer copies and scans every
retained applied sparse weight/provenance row before returning capped audit
rows. The production path emits
...
ledger rows only for selected hashes, and blocks exact audit review when the
source window is truncated. It reports no live-tick work, no every-token work,
no mutation/plasticity, no hidden replay-text language reasoning, and no
...
GPU allocation stayed `0`.

Long protection runs:

`python -m marulho.evaluation.continuous_runtime_stress_benchmark --checkpoint reports\column_scheduler_20260618\checkpoints\active-pressure-scheduler-65536-seeded.pt --output reports\bounded_replay_window_20260620\hotpath-active-pressure-65536-524288-i32-synapse-provenance-audit-source-window.json --target-tokens 524288 --tick-tokens 128 --quantum-tokens 16 --source-concept-observation-tick-interval 4 --timeout-seconds 900 --sample-interval-seconds 0.05 --host-truth-sync-interval-tokens 32`

The first run succeeded at `6441.926 tokens/sec`, `tick_duration_ms.p95=19.505`,
and `train_compute=0.127116 ms/token`, with bounded `12/65536` route rows and
zero graph/native sequence failures, but is kept as secondary evidence because
velocity reported contention.
...
`train_compute=0.127184 ms/token`, `prepare_training=0.006068 ms/token`,
`finalize_total=0.005747 ms/token`, and prewarm `239.005 s`. Runtime Truth
kept route scoring at `12/65536` input rows and `10` output candidates, cached
`65526` transition rows, kept `state_transition_runs_all_columns=false`, and
recorded zero graph/native sequence failures. Contention was `not_observed`
...
and rollout/server binding status now share one bounded newest-first
transition-memory source-window helper. The old status-local insertion-order
mapping helper is removed, so status and plasticity runtime-state views use one
source-window rule. The status path reads at most `32`
...
`python -m marulho.evaluation.status_transition_memory_source_window_benchmark --entry-count 2048 --runs 25 --output C:\Users\thiag\Documents\MARULHO_reports\bounded_replay_window_20260623\status-transition-memory-source-window-recent-helper.json`

Result: `pass=true`; the recency quality gate inserted `32` stale invalid
rows first and `32` valid recent rows last, then verified the selected source
window used the valid recent rows (`invalid_synapse_key_count=0`,
`source_sparse_transition_weight_count=32`,
`source_synapse_provenance_count=32`). The maintained path still read `256`
rows across four projections versus `10240` retired repeated broad rows
(`40x` less source work), reduced mean latency from `87.635592 ms` to
`12.310596 ms` (`7.118712x`), kept bounded Python peak at `0.047835 MiB` mean
versus `1.269724 MiB`, and used `0.0 MiB` CUDA allocation/reservation.
...
`python -m marulho.evaluation.continuous_runtime_stress_benchmark --checkpoint reports\column_scheduler_20260618\checkpoints\active-pressure-scheduler-65536-seeded.pt --output C:\Users\thiag\Documents\MARULHO_reports\bounded_replay_window_20260623\hotpath-active-pressure-65536-524288-i32-status-transition-recent-helper-default-nosample.json --target-tokens 524288 --tick-tokens 128 --quantum-tokens 16 --source-concept-observation-tick-interval 4 --timeout-seconds 900 --sample-interval-seconds 0.05 --environment-sample-interval-seconds 0 --host-truth-sync-interval-tokens 32 --profile-trainer-stages`

Refresh accepted: `success=true`, `524288` tokens in `86.595054 s`,
`6054.480 tokens/sec`, `tick_duration_ms.p95=21.719`,
`train_compute=0.133249 ms/token`, `prepare_training=0.006702 ms/token`, and
`finalize_total=0.006722 ms/token`. Prewarm took `244.570 s`. Runtime Truth
kept route scoring at `12/65536` input rows and `10` output candidates, cached
`65526` transition rows, kept `state_transition_runs_all_columns=false`, and
...
transition memory through
`bounded_snn_language_plasticity_runtime_transition_memory_source_window.v1`
instead of deep-copying the full retained maps. The returned window is capped at
`64` sparse-transition rows plus `64` synapse-provenance rows, while retained
counts remain visible for exact-readiness gating. It is CPU archival/status
projection work, not replay execution, mutation, raw-text processing, hidden
language reasoning, live-tick work, or every-token slow-memory admission.

Focused quality and latency benchmark:

`python -m marulho.evaluation.plasticity_runtime_state_source_window_benchmark --entry-count 65536 --runs 7 --output reports\bounded_replay_window_20260621\plasticity-runtime-state-source-window.json`

Result: `pass=true`; active source reads were `256` rows versus `262144` rows
in the benchmark-local retired full snapshot (`1024x` less source work). Mean
snapshot latency fell from `752.314014 ms` to `7.770271 ms` (`96.819528x`).
Python traced peak allocation was `0.110454 MiB` versus `12.287186 MiB`; CUDA
allocation/reservation deltas stayed `0` on the RTX 3060. Runtime Truth reports
CPU archival/source/lookup placement, no global scan, no replay, no raw text,
...
First run: `success=true`, `524288` tokens in `92.911295 s`,
`5642.888 tokens/sec`, `tick_duration_ms.p95=23.533`,
`train_compute=0.141701 ms/token`, `prepare_training=0.007814 ms/token`, and
`finalize_total=0.007287 ms/token`. Prewarm took `396.310 s`. Runtime Truth
kept route scoring at `12/65536` input rows and `10` output candidates, cached
`65526` transition rows, kept `state_transition_runs_all_columns=false`, and
recorded zero graph/native sequence failures. Velocity reported no observed
contention (`cpu max=39%`, `gpu max=13%`, GPU memory utilization max `18%`);
RTX 3060 memory moved `2092->2098 MiB`.

`python -m marulho.evaluation.continuous_runtime_stress_benchmark --checkpoint reports\column_scheduler_20260618\checkpoints\active-pressure-scheduler-65536-seeded.pt --output reports\bounded_replay_window_20260621\hotpath-active-pressure-65536-524288-i32-plasticity-runtime-state-source-window-rerun.json --target-tokens 524288 --tick-tokens 128 --quantum-tokens 16 --source-concept-observation-tick-interval 4 --timeout-seconds 900 --sample-interval-seconds 0.05 --host-truth-sync-interval-tokens 32`

Rerun: `success=true`, `524288` tokens in `91.397784 s`,
`5736.332 tokens/sec`, `tick_duration_ms.p95=22.626`,
`train_compute=0.139447 ms/token`, `prepare_training=0.007651 ms/token`, and
`finalize_total=0.007188 ms/token`. Prewarm took `355.860 s`. Runtime Truth
again kept route scoring at `12/65536`, cached `65526` transition rows, kept
`state_transition_runs_all_columns=false`, and recorded zero graph/native
...
inside recent anchor capture. `DualMemoryStore.collect_recent_entry_indices(...)`
still selects the bounded recent source window, and
`DualMemoryStore.recent_anchor_capture_row(...)` now serves selected bucket
rows under `bounded_recent_anchor_capture_row.v1`; trainer code no longer
indexes `slow_bucket_ids` directly.
...
Result: `status=passed`; anchor capture read `64` store-owned rows, captured
`64` anchors, had zero invalid anchor rows, reported
`direct_slow_memory_bucket_reads_retired=true`, used CPU archival placement,
loaded no raw replay text, performed no hidden language reasoning, did no
live/every-token work, used no global scan, and had `0.0 MiB` CUDA allocation
delta. Mean capture latency was `1.743 ms`, p95 `2.071 ms`.
...
`finalize_total=0.006956 ms/token`. Prewarm took `370.179 s` and
`full_warm_ready=true` before measurement. Runtime Truth kept route scoring
bounded at `12/65536`, cached `65526` transition rows, kept
`state_transition_runs_all_columns=false`, selected CUDA on the RTX 3060, and
recorded zero graph/native/sequence failures. Velocity reported no observed
...
## Query Readout Comparator-Removal Protection

This run protects the live tick after removing benchmark-local report-dropping
context readout, eager candidate text payload loading, direct runtime concept
archive lookup, and fragment-only episode readout comparators.

`python -m marulho.evaluation.continuous_runtime_stress_benchmark --checkpoint reports\column_scheduler_20260618\checkpoints\active-pressure-scheduler-65536-seeded.pt --output ..\..\MARULHO_reports\bounded_replay_window_20260624\hotpath-active-pressure-65536-524288-i32-query-readout-comparators-removed-default-nosample.json --target-tokens 524288 --tick-tokens 128 --quantum-tokens 16 --source-concept-observation-tick-interval 4 --timeout-seconds 900 --sample-interval-seconds 0.05 --environment-sample-interval-seconds 0 --host-truth-sync-interval-tokens 32 --profile-trainer-stages`

Result: `success=true`, `524288` tokens in `79.605266 s` at
`6586.097 tokens/sec`, p95 tick `18.732 ms`,
`train_compute=0.123762 ms/token`, `prepare_training=0.005740 ms/token`, and
`finalize_total=0.005744 ms/token`. Prewarm took `206.655 s` outside the
measured throughput window. Runtime Truth kept route scoring bounded at
`12/65536`, output candidates at `10`, and transition caching at `65526` rows;
`state_transition_runs_all_columns=false`, graph/native sequence failures were
zero, and velocity sampled no contention. CPU max was `39%`, GPU max was `9%`,
and RTX 3060 memory stayed flat at `1886 MiB`.

## Readout-Ledger Benchmark Comparator-Removal Protection

This run protects the live tick after replacing the readout-ledger normalization
benchmark with maintained-only bounded source-window evidence. The old
full-materialized normalization/store comparators and broad-normalized
per-boundary comparators are absent from repo-local executable code.

`python -m marulho.evaluation.continuous_runtime_stress_benchmark --checkpoint reports\column_scheduler_20260618\checkpoints\active-pressure-scheduler-65536-seeded.pt --output ..\..\MARULHO_reports\bounded_replay_window_20260624\hotpath-active-pressure-65536-524288-i32-readout-ledger-normalization-comparators-removed-default-nosample.json --target-tokens 524288 --tick-tokens 128 --quantum-tokens 16 --source-concept-observation-tick-interval 4 --timeout-seconds 900 --sample-interval-seconds 0.05 --environment-sample-interval-seconds 0 --host-truth-sync-interval-tokens 32 --profile-trainer-stages`

Result: `success=true`, `524288` tokens in `80.568606 s` at
`6507.349 tokens/sec`, p95 tick `19.722 ms`,
`train_compute=0.124647 ms/token`, `prepare_training=0.006070 ms/token`, and
`finalize_total=0.005990 ms/token`. Prewarm took `238.250 s` outside the
measured throughput window. Runtime Truth kept route scoring bounded at
`12/65536`, output candidates at `10`, and transition caching at `65526` rows;
`state_transition_runs_all_columns=false`, graph/native sequence failures were
zero, and velocity sampled no contention. CPU max was `41%`, GPU max was
`10%`, and RTX 3060 memory stayed flat at `1968 MiB`.
