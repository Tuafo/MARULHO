...
- Executor transaction tests: `python -m pytest tests\test_structural_mutation_executor.py`
...
- Post-transaction-change long throughput run: `python -m marulho.evaluation.continuous_runtime_stress_benchmark --checkpoint reports\column_scheduler_20260617\checkpoints\active-pressure-scheduler-65536-seeded.pt --output reports\growth_pruning_20260617\checkpointed-candidate-transaction-65536-262144-i32.json --target-tokens 262144 --tick-tokens 128 --quantum-tokens 16 --host-truth-sync-interval-tokens 32 --timeout-seconds 1200 --sample-interval-seconds 0.5`
...
Latest post-transaction-change evidence: `reports\growth_pruning_20260617\checkpointed-candidate-transaction-65536-262144-i32.json` processed `262144` tokens at `6144.705 tokens/sec` on `NVIDIA GeForce RTX 3060`, with `velocity_environment.contention.verdict=not_observed`, `train_compute=0.131836 ms/token`, `prepare_training=0.006386 ms/token`, `finalize_total=0.006442 ms/token`, and `tick_duration_ms.p95=20.928`. Runtime Truth stayed bounded: `route_input_rows_scored=12/65536`, `route_output_candidate_count=10`, `state_transition_cached_count=65526`, `state_transition_runs_all_columns=false`, `observed_filtered_deep_sleep_total=510`, `observed_filtered_memory_pressure_total=2`, `observed_fallback_count=0`, `native_sequence_loop_token_count=262128`, and `native_sequence_loop_failure_count=0`. This keeps throughput at the current 6k-ish level after adding executor-side candidate provenance/tombstones; it is not a new speed ceiling.
...
Executor transaction evidence: `tests\test_structural_mutation_executor.py` covers accepted binding-hub topology mutation, no-delta rejection, commit-failure rollback, over-budget rollback, missing-approval block, tampered target rejection, and tampered candidate-provenance rejection. Blocked/rejected executor responses now include `candidate_provenance`, `rollback_artifact`, and `tombstone_manifest`; service endpoint coverage confirms a blocked application leaves runtime revision and action history unchanged while returning the candidate tombstone.
