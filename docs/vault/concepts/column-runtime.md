...
State transition scope is now its own truth surface. A tick can have bounded awake candidates and cached non-awake votes while still mutating dense column state. Runtime Truth therefore exposes `state_transition_mode`, `state_transition_column_count`, `state_transition_runs_all_columns`, and `state_transition_fallback_reason` beside route/vote/sleep evidence. The service read model only projects these fields; it does not decide sleep, wake, route candidates, or transition scope.
