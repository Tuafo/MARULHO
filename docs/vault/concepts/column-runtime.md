...
update replay counts, consolidation levels, capture tags, and EMAs, but a
...
Slow-memory admission now follows the same selected-window boundary. Fixed cadence is no longer a write path from retained `train_step`: first-token retained/fallback records and selected strong-capture events are the only live archive admissions. Ordinary `slow_memory_archive_interval_tokens` hits record `cadence_deferred` in the cognitive boundary controller and increment skip pressure instead of calling `DualMemoryStore.update(...)` or awake-ripple tagging. The fixed-cadence retirement benchmark `reports/bounded_replay_window_20260620/slow-memory-fixed-cadence-admission-retired.json` kept `1` first-token archive and retired `17` projected fixed-cadence writes over `256` tokens, while the refreshed strong-capture benchmark kept `17` selected strong archives. The current maintained-only reports `..\..\MARULHO_reports\bounded_replay_window_20260624\slow-memory-fixed-cadence-projection-removed.json` and `..\..\MARULHO_reports\bounded_replay_window_20260624\strong-capture-admission-projection-removed.json` remove the fixed-cadence and every-strong projection objects from repo-local benchmark reports, keep CPU archival placement, and record absence fields for both retired admission projections. The current 65536-column `524288`-token protection run stayed same-band at `5957.637 tokens/sec`, bounded route scoring at `12/65536`, cached `65526` transition rows, deferred `2048` cadence hits, kept native sequence-loop and burst-replay failure counts at `0`, reported no observed before/after contention (`gpu max=12%`), and kept RTX 3060 memory `2047->2046 MiB`. This makes fixed cadence a Runtime Truth/maintenance signal, not a second replay-admission implementation beside the wake plan.
...
offline recovery. Missing IDs, dirty caches, or missing row-update APIs now
report deferred recovery and do not call `add()+rebuild()` inside selected
replay. The maintained-only `65536`-row benchmark
`..\..\MARULHO_reports\bounded_replay_window_20260623\sleep-replay-routing-index-legacy-baseline-removed-rerun.json`
records the old full-rebuild diagnostic as absent, updates `16` selected rows,
defers `1` missing row, and keeps exact top-1 recall with `9.892320 ms` mean
bounded refresh latency (`median=5.693200 ms`). The sharded report passed with
`16` direct shard and merged updates at `19.582580 ms` mean latency. Archival
storage and row metadata stay CPU-resident while active routing tensors stay on
CUDA, with Python trace-memory peak below `0.085 MiB`. Service surfaces may
report the refresh mode, row lookup mode, deferred recovery flag, and
skipped-update count, but they do not select replay rows or decide routing
maintenance. The current `524288`-token protection run stayed in band at
`6097.811 tokens/sec` with bounded `12/65536` route rows, cached `65526`
transition rows, CPU max `32%`, GPU max `15%`, RTX 3060 memory
`1825->1824 MiB`, and zero graph/native sequence failures.

Sleep replay now has the same bounded-window accounting. `DualMemoryStore.select_replay_window(...)` reports `bounded_replay_window_selection.v1` before any replay mutation. Deep sleep passes checkpointed `column_anchors` as candidate bucket ids, so a positive-pressure replay window scores only entries reachable through the memory store's bucket-to-entry index. That bucket-indexed path now caps candidates before scoring by recent round-robin across anchor buckets and records `candidate_window_policy=recent_bucket_round_robin_candidate_pool`, `candidate_window_limit`, `candidate_index_available_count`, and scored `candidate_index_count`; a hot bucket can make more entries available, but it cannot make the selector score every stored entry. `DualMemoryStore.recall_replay_window(...)` reports `bounded_replay_window_recall.v1` as a non-mutating slow-path operator over the selected entries: routing-key recall, nearest input-pattern recall, and the bounded Hopfield-style recalled input projection stay CPU-resident, `runs_live_tick=false`, and no plasticity is applied. The report records candidate bucket ids/count, candidate entries scored, selected count, CPU archival/score/active recall placement, `recalled_input_pattern_distance`, `input_attention_entropy`, and whether the lower-level selector used `bucket_indexed_candidate_window` or blocked unscoped selection with `bucket_index_scope_required`. There is no runtime diagnostic global score/candidate branch. Production deep replay no longer mutates from the unscoped global scorer: no-anchor and zero-pressure bucket cases record `unscoped_global_fallback_retired=true` and apply `0` replay updates. The former list-only replay/SFA helpers are now removed: callers use `select_replay_window(...)` and `sample_for_sfa_with_report(...)` so bounded reports are retained; unscoped SFA now reports `selected_replay_window_required`. The capped-window long run processed `262144` tokens at `6148.125 tokens/sec`, kept route scoring bounded at `12/65536`, cached `65526` transition rows, reported no observed contention, held GPU memory flat at `1848 MiB`, and had zero graph/native/sequence failures.
...
The 2026-06-30 bounded input-projection refresh makes the local recall quality metric match the intended modern-Hopfield operator. `..\..\MARULHO_reports\bounded_replay_window_20260630\hf-bounded-hopfield-input-recall-512-final\summary.json` passed HF recall after Task B and after consolidation with `mean_recalled_input_pattern_distance=0.0`, `mean_input_pattern_distance=0.0`, one selected anchor bucket, CPU recall/score/archive placement, no full scan, no live tick, and no recall mutation. The same slow-window guard accepted `3` replay updates and improved Task-A reconstruction from `0.0258607082` to `0.0246669967`; checkpoint reload preserved the new `bounded_hopfield_input_projection_cpu` report. The long live-tick protection run `..\..\MARULHO_reports\bounded_replay_window_20260630\hotpath-active-pressure-65536-524288-i32-bounded-hopfield-input-recall.json` stayed same-band at `5687.544 tokens/sec`, p95 `23.405 ms`, `train_compute=0.140331 ms/token`, route scoring `12/65536`, `state_transition_runs_all_columns=false`, zero graph/native sequence failures, no observed contention, and flat RTX 3060 memory at `1650 MiB`.
...
repair calls `DualMemoryStore.sleep_repair_replay_row(...)`, which has no text
payload option.
...
Current executable scheduler benchmarks no longer run the old all-column
comparator. `column_scheduler_benchmark.v2` reports the maintained scoped path
directly and keeps historical all-column A/Bs in documentation only. Runtime
Truth remains the active guard: bounded candidates, bounded update counts,
`runs_all_columns=false`, and structural-review continuation evidence.
...
tokens with a max selected gap of `16`. The current maintained-only refresh
removes the every-strong projection object too; the report carries
`retired_every_strong_admission_absence.implementation_present=false` and
projects removed writes only inside `memory_budget`.
...
The generic `DualMemoryStore.replay_entry(...)` API is removed. Production row
access is purpose-named: `sleep_repair_replay_row(...)` is the mutating
tensor-only sleep repair row, `replay_recall_row(...)` is read-only sleep
recall, and `query_match_row(...)` is query/source-bank/context recall. Text
payloads only enter through explicit bounded query/source rows after candidate
selection.

Fresh external reports under
`..\..\MARULHO_reports\bounded_replay_window_20260622\` passed the cleanup gate:
...
`query_row_reader=DualMemoryStore.replay_recall_row`. HF replay-query collection
now reports `direct_slow_memory_input_pattern_reads_retired=true` after bounded
anchor-bucket selection; its smoke runner kept the memory-consolidation gate
passing but did not promote the bounded recall quality gate. The `524288`-token
protection rerun stayed in the same noisy band at `5943.110 tokens/sec`, and
the telemetry-retirement rerun stayed same-band at `5819.770 tokens/sec` with
route scoring bounded to `12/65536`, `65526` cached transition rows, `524288`
skipped graph consolidation lookups, CUDA active on the RTX 3060, zero native
sequence-loop fallback, and RTX memory flat at `1934 MiB`. This boundary does
not change the live column tick or the explicit mutating replay/consolidation
path.
