...
---
        ...
        `snn_language_plasticity_runtime_state.v1` now exposes canonical `language_*` and `readout_*` evidence names for capacity mutation, newborn-neuron integration, critical-period learning, and newborn-synapse pruning. The readout-capacity and readout-newborn developmental chains are canonical end to end, and active runtime-state field labels no longer describe current readout evidence as legacy aliases. Legacy `thought_*` keys are retired-path evidence only and are not a claim that MARULHO has a hidden text-thought substrate.

        Surface, readout-memory, readout-consolidation, readout-structural-plasticity, readout-capacity mutation, newborn-neuron integration, critical-period learning, maturation review, and newborn-synapse pruning APIs now use canonical readout routes and canonical readout request fields. The surface, memory, consolidation, and structural route families are explicitly `snn-language-readout-surface-*`, `snn-language-readout-memory-*`, `snn-language-readout-consolidation-*`, and `snn-language-readout-structural-plasticity-*`; the capacity route family is explicitly `snn-language-readout-capacity-mutation-*`; and the developmental route family is explicitly `snn-language-readout-newborn-*`. These routes delegate to the same reviewed ledger/executor gates and do not introduce a new mutation path. Thought-era public/internal route vocabulary, generic readout route aliases, and the API mapper that translated readout payloads back into thought-era names are no longer part of the active API.
