...
- Operational-manifold and open-world SNN agent work plus Cortex retirement imply status vocabulary should identify retired paths as compatibility evidence, not active liveness dependencies. `retired_runtime_path` should be canonical while `cortex_*` remains a temporary alias for consumers being migrated.
