...
**SNN Dense Readout Decoder-Probe Design** — the read-only design for connecting a post-training dense SNN language readout to HECSN's bounded Spike Language Decoder Probe. It requires post-training evaluation, grounded readout slots, sparse population-code evidence, temporal-state evidence, tensor device evidence, and explicit free-form generation lockout before any decoder-probe preflight can be reviewed; it cannot sample text, generate language, execute actions, mutate runtime state, write checkpoints, or claim cognition.
...
**SNN Dense Label-Candidate Evidence Ledger** — the HECSN-owned append-only audit lane for operator-confirmed dense-readout label-candidate reviews. It records review hash, decoder execution hash, grounded labels, label hash, tensor-device evidence, sparse activity count, state revision, and operator identity for later inspection, while remaining ineligible for replay memory, plasticity, facts, actions, checkpoints, free-form generation, or cognition-substrate claims by itself.
...
**SNN Dense Label-Candidate Calibration Update Application** — the checkpoint-backed mutation that installs bounded post-hoc dense label calibration parameters into HECSN-owned readout state after a ready preflight and operator confirmation. It records applied calibration lineage and advances runtime revision, but it cannot persist dense weights, train, replay, apply plasticity, promote facts/actions, generate language, or claim cognition.
