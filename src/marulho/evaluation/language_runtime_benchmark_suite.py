...
BRAIN_INSTALLED_GENERATION_SURFACE = (
    "marulho_language_brain_installed_generation_evidence.v1"
)
...
def _read_brain_installed_generation_report(path: str | Path) -> dict[str, Any]:
    report_path = Path(path)
    with report_path.open("r", encoding="utf-8") as handle:
        payload = json.load(handle)
    if not isinstance(payload, dict):
...
def _valid_brain_installed_generation_report(report: Mapping[str, Any]) -> bool:
    gate = (
        report.get("promotion_gate")
        if isinstance(report.get("promotion_gate"), Mapping)
        else {}
    ...
    coherence = (
        report.get("generation_coherence")
        if isinstance(report.get("generation_coherence"), Mapping)
        else {}
    )
    summary = (
        report.get("generation_summary")
        if isinstance(report.get("generation_summary"), Mapping)
        else {}
    )
    checkpoint = (
        report.get("brain_checkpoint")
        if isinstance(report.get("brain_checkpoint"), Mapping)
        else {}
    )
    return (
        report.get("artifact_kind") == BRAIN_INSTALLED_GENERATION_ARTIFACT_KIND
        and report.get("surface") == BRAIN_INSTALLED_GENERATION_SURFACE
        and report.get("report_status") == "final"
        and report.get("status") == "final"
        and report.get("runtime_owner") == "MarulhoBrain"
        and report.get("active_language_path") == "marulho_lm_head"
        ...
        and int(summary.get("case_count", 0) or 0) > 0
        and gate.get("loaded_installed_brain_checkpoint") is True
        and gate.get("brain_checkpoint_restore_verified") is True
        and gate.get("batch_tokenizer_matches_installed_runtime") is True
        and gate.get("generation_runs_through_marulho_brain") is True
        and gate.get("status_read_mutation_absent") is True
        and gate.get("external_llm_absent") is True
        and gate.get("service_owned_cognition_absent") is True
        and gate.get("promotes_runtime_claim") is False
...
def _brain_installed_generation_summary(
    report: Mapping[str, Any],
) -> dict[str, Any]:
    summary = (
        report.get("generation_summary")
    ...
    return {
        ...
        "brain_checkpoint_path": report.get("brain_checkpoint_path"),
        ...
        "mean_prefix_match_fraction": summary.get("mean_prefix_match_fraction"),
        "mean_printable_fraction": summary.get("mean_printable_fraction"),
        "mean_distinct_bigram_fraction": summary.get(
        ...
        "generation_runs_through_marulho_brain": bool(
            gate.get("generation_runs_through_marulho_brain", False)
        ),
        "status_read_mutation_absent": bool(
            gate.get("status_read_mutation_absent", False)
        ),
        "coherence_report_status": coherence.get("status"),
        "promotes_generation_quality_claim": False,
        "promotes_runtime_claim": False,
    }


def _language_brain_installed_generation_evidence(
    reports: Sequence[Mapping[str, Any]],
) -> dict[str, Any]:
    valid_reports = [
        dict(report)
        for report in reports
        if _valid_brain_installed_generation_report(report)
    ]
    best_report = max(
        valid_reports,
        key=lambda item: (
            int(
                (item.get("generation_summary") or {}).get("case_count", 0)
                if isinstance(item.get("generation_summary"), Mapping)
                else 0
            ),
            float(
                (item.get("generation_summary") or {}).get("case_pass_rate", 0.0)
                if isinstance(item.get("generation_summary"), Mapping)
                else 0.0
            ),
        ),
    ...
    return {
        "surface": "marulho_language_brain_installed_generation_saved_evidence.v1",
        "report_count": len(reports),
        "valid_report_count": len(valid_reports),
        "brain_installed_generation_available": best_report is not None,
        "best_report": (
            None
            if best_report is None
            else _brain_installed_generation_summary(best_report)
        ),
        "missing_evidence": (
...
def _brain_installed_generation_repair_long_run_alignment_evidence(
    brain_installed_generation_repair_evidence: Mapping[str, Any],
    long_run_evidence: Mapping[str, Any],
) -> dict[str, Any]:
    best_report = (
        brain_installed_generation_repair_evidence.get("best_report")
        if isinstance(
            brain_installed_generation_repair_evidence.get("best_report"),
            Mapping,
        )
    ...
    repaired_checkpoint = best_report.get("repaired_brain_checkpoint_path")
    repaired_checkpoint_key = _normalized_evidence_path(repaired_checkpoint)
    long_reports = _long_run_report_summaries(long_run_evidence)
    matching_reports = [
        report
        for report in long_reports
        if repaired_checkpoint_key
        and _normalized_evidence_path(report.get("checkpoint_path"))
    ...
    evidence_available = bool(repaired_checkpoint_key and matching_long_reports)
    controlled_house_required = bool(
        long_run_evidence.get("controlled_decode_house_scale_gate_reached")
    )
    missing: list[str] = []
    if not evidence_available:
        missing.append("same_checkpoint_brain_generation_repair_long_run")
    if controlled_house_required and not matching_controlled_house_reports:
        missing.append(
            "same_checkpoint_brain_generation_repair_controlled_decode_house_scale"
        )
    return {
        "surface": (
            "marulho_language_brain_installed_generation_repair_"
            "long_run_alignment.v1"
        ),
        "repaired_brain_checkpoint_path": repaired_checkpoint,
        "long_run_checkpoint_paths": [
            report.get("checkpoint_path") for report in long_reports
        ],
        "matching_report_count": len(matching_reports),
        "same_checkpoint_long_run_available": evidence_available,
        "same_checkpoint_house_scale_available": bool(matching_house_reports),
        "same_checkpoint_controlled_decode_available": bool(
            matching_controlled_reports
        ),
        "same_checkpoint_controlled_decode_house_scale_available": bool(
            matching_controlled_house_reports
...
def run_language_runtime_benchmark_suite(
    *,
    ...
    brain_installed_generation_reports = [
        _read_brain_installed_generation_report(path)
        for path in brain_installed_generation_evidence_paths
    ]
    brain_installed_generation_evidence = (
        _language_brain_installed_generation_evidence(
            brain_installed_generation_reports
        )
    )
    ...
    best_brain_installed_generation_repair = (
        brain_installed_generation_repair_evidence.get("best_report")
        if isinstance(
            brain_installed_generation_repair_evidence.get("best_report"),
            Mapping,
    ...
    brain_installed_generation_repair_coherence_available = bool(
        int(best_brain_installed_generation_repair.get("case_count", 0) or 0) > 0
        and int(
            best_brain_installed_generation_repair.get(
                "post_passed_case_count",
                0,
            )
            or 0
        )
        == int(best_brain_installed_generation_repair.get("case_count", 0) or 0)
        and int(
            best_brain_installed_generation_repair.get(
                "regressed_prompt_count",
                0,
            )
            or 0
        )
    ...
    generation_report_missing = (
        ...
        if (
            ...
            or brain_installed_generation_repair_coherence_available
        )
        ...
        evidence={
            ...
            **generation_coherence_evidence,
            ...
            "brain_installed_generation_repair_satisfies_grounded_coherence": (
                brain_installed_generation_repair_coherence_available
            ),
    ...
    brain_installed_generation_repair_long_run_alignment = (
        _brain_installed_generation_repair_long_run_alignment_evidence(
            brain_installed_generation_repair_evidence,
            long_run_evidence,
        )
    )
    ...
    if (
        brain_installed_generation_repair_coherence_available
        and not long_run_missing
        and brain_installed_generation_repair_long_run_alignment["missing_evidence"]
    ):
        generation_category["status"] = "smoke_only"
        generation_category["missing_evidence"].extend(
            brain_installed_generation_repair_long_run_alignment["missing_evidence"]
        )
    ...
    report = {
        ...
        "promotion_gate": {
            ...
            "generation_controlled_decode_house_scale_aligned": bool(
                ...
                or brain_installed_generation_repair_long_run_alignment.get(
                    "same_checkpoint_controlled_decode_house_scale_available"
                )
            ),
            "brain_installed_generation_repair_controlled_decode_house_scale_aligned": bool(
                brain_installed_generation_repair_long_run_alignment.get(
                    "same_checkpoint_controlled_decode_house_scale_available"
                )
            ),
...
def main() -> int:
    ...
    parser.add_argument(
        "--brain-installed-generation-evidence",
        type=Path,
        action="append",
        default=[],
        help=(
            "Existing marulho_language_brain_installed_generation_evidence JSON "
            "report."
        ),
    )
    parser.add_argument(
