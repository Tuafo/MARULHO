...
The deleted V70 preflight and quality runner established a mechanically healthy
but terminally worse all-local macro cortex: 5.7188 versus 5.5000 heldout loss
...
V80 Stage 0's deleted preflight compared the historical three-call episode with
one batch-packed call over the same independent segments. Packing reached
22.228k versus 20.489k positions/s, but the 1.0849x ratio missed the frozen 1.10x
gate and BF16 scalar-loss delta 0.003906 missed the 0.002 parity bound. Gradient
cosine was 0.999938 and post-update heldout loss matched exactly, so the idea was
sound but not valuable enough to change the long-run numerical path. Decision:
...
V61's two-step nonlinear fast learner is terminally negative and deleted. All
12 slow tensors receive gradients and training sustains 50,392 source positions/s
with 1.41 GB peak allocation. The first final inner update improves 35.71 to
18.28 reconstruction loss, but the second diverges to 5,640.99. Untrained true,
learned no-write, shuffled, true, and oracle-short views all score 0/256, with
accepted-answer containment only 1/1/0/0/1. Decision:
`retire_v61_final_residual_nonlinear_fast_learner`; no checkpoint survives.
Report SHA-256 is
