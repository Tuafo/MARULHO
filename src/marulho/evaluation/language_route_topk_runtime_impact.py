"""Measure routed LM forward impact from route/vote top-k Triton selection."""
...
SURFACE = "marulho_language_route_topk_runtime_impact.v1"
ARTIFACT_KIND = "marulho_language_route_topk_runtime_impact"
_MIN_ROWS_ENV = "MARULHO_LANGUAGE_ROUTE_TOPK_TRITON_MIN_ROWS"


@dataclass(frozen=True)
class RouteTopKRuntimeImpactConfig:
    ...
    warmup_steps: int = 5
    repeats: int = 50
    fallback_min_rows: int = 1_000_000_000
    triton_min_rows: int = 1
    device: str = "auto"
    seed: int = 20260704
...
@contextmanager
def _route_topk_min_rows(value: int) -> Iterator[None]:
    previous = os.environ.get(_MIN_ROWS_ENV)
    os.environ[_MIN_ROWS_ENV] = str(max(1, int(value)))
    try:
        yield
    finally:
        if previous is None:
            os.environ.pop(_MIN_ROWS_ENV, None)
        else:
            os.environ[_MIN_ROWS_ENV] = previous
...
def _run_arm(
    ...
    *,
    ...
    try:
        ...
        model.eval()
        with _route_topk_min_rows(min_rows), torch.inference_mode():
            ...
            stats_after = language_route_topk_triton_stats()
        ...
        stats_delta = language_route_topk_triton_stats_delta(
            {
                "triton_available": stats_after.get("triton_available"),
        ...
        return {
            "surface": "marulho_language_route_topk_runtime_arm.v1",
            "name": name,
            "success": True,
            "failure_reason": None,
            "route_topk_min_rows": int(min_rows),
            ...
            "route_topk_triton_stats_delta": stats_delta,
...
def _comparison(
    ...
    *,
    ...
    triton_stats = triton_arm.get("route_topk_triton_stats_delta", {})
    fallback_stats = fallback.get("route_topk_triton_stats_delta", {})
    ...
    if triton_success and fallback_success and triton_kernel_used and fallback_used_torch:
        evidence_status = "measured_triton_vs_torch_route_topk_forward"
    elif triton_success and fallback_success:
        evidence_status = "measured_without_route_topk_triton_use"
    else:
        evidence_status = "runtime_impact_measurement_failed"
    return {
        "surface": "marulho_language_route_topk_runtime_comparison.v1",
        ...
        "fallback_route_topk_torch_used": fallback_used_torch,
        "triton_route_topk_kernel_used": triton_kernel_used,
...
def run_language_route_topk_runtime_impact(
    *,
    output_path: str | Path,
    config: RouteTopKRuntimeImpactConfig | None = None,
) -> dict[str, Any]:
    cfg = config or RouteTopKRuntimeImpactConfig()
    ...
    fallback_report, fallback_logits = _run_arm(
        "torch_route_topk_policy_fallback",
        min_rows=int(cfg.fallback_min_rows),
    ...
    triton_report, triton_logits = _run_arm(
        "triton_route_topk_enabled",
        min_rows=int(cfg.triton_min_rows),
        base_state=base_state,
        batch=batch,
        config=cfg,
        tokenizer=tokenizer,
    ...
    report = {
        ...
        "split": split_report,
        "arms": {
            "torch_route_topk_policy_fallback": fallback_report,
            "triton_route_topk_enabled": triton_report,
        },
        ...
        "review": {
            ...
            "next_experiment": (
                "tighten route-topk min-row policy only after repeated forward "
                "impact wins and sustained generation confirms no streaming regression"
            ),
        },
        "promotion_gate": {
            ...
            "route_topk_triton_used": bool(
                comparison["triton_route_topk_kernel_used"]
            ),
...
def main() -> int:
    ...
    parser.add_argument("--fallback-min-rows", type=int, default=1_000_000_000)
    parser.add_argument("--triton-min-rows", type=int, default=1)
    ...
    config = RouteTopKRuntimeImpactConfig(
        ...
        triton_min_rows=args.triton_min_rows,
        device=args.device,
    )
