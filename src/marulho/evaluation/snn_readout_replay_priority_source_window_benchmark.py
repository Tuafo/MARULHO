"""Benchmark bounded SNN readout replay-priority source windows."""
...
def _seed_events(*, retention_count: int) -> list[dict[str, Any]]:
    count = max(SNN_READOUT_REPLAY_PRIORITY_SOURCE_WINDOW_LIMIT + 1, int(retention_count))
    events: list[dict[str, Any]] = []
    for source_index in range(count):
        if source_index < 4:
            label = "recent-high-signal-readout"
            weights_hash = "recent-high-signal-weights"
        elif source_index >= count - 4:
            label = "old-retained-readout"
            weights_hash = "old-retained-weights"
        else:
            label = f"readout-distractor-{source_index}"
            weights_hash = f"weights-{source_index % 23}"
        events.append(
...
def _legacy_full_retained_priority(
    events: Sequence[Mapping[str, Any]],
    *,
    limit: int,
) -> dict[str, Any]:
    """Diagnostic-only copy of the retired all-retained scoring shape."""
    ...
    selected = [
        {**candidate, "rank": rank}
        for rank, candidate in enumerate(candidates[: max(0, int(limit))], start=1)
    ]
    return {
        "surface": "diagnostic_legacy_snn_readout_full_retained_priority.v1",
...
def run_benchmark(args: argparse.Namespace) -> dict[str, Any]:
    events = _seed_events(retention_count=int(args.retention_count))
    lock = RLock()
    runtime_state = RuntimeState(lock=lock)
    ledger_state: dict[str, Any] = {
        "events": deepcopy(events),
        "total_recorded_count": int(len(events)),
        "last_recorded_at": events[0]["recorded_at"] if events else None,
    }
    ledger = SNNLanguageReadoutEvidenceLedger(
        lock=lock,
        runtime_state=runtime_state,
        ledger_state=lambda: ledger_state,
        limit=int(len(events)),
    )

    rss_before = _process_rss_mb()
    bounded, bounded_samples = _timed_runs(
        runs=int(args.runs),
        fn=lambda: ledger.replay_priority(limit=int(args.limit)),
    )
    legacy, legacy_samples = _timed_runs(
        runs=int(args.runs),
        fn=lambda: _legacy_full_retained_priority(
            ledger_state["events"],
    ...
    tracemalloc.start()
    _ = ledger.replay_priority(limit=int(args.limit))
    ...
    bounded_top_hash = (
        bounded_candidates[0].get("readout_evidence_hash")
        if bounded_candidates
        else None
    )
    legacy_top_hash = (
        legacy_candidates[0].get("readout_evidence_hash")
        if legacy_candidates
        else None
    )
    bounded_top_labels = (
        list(bounded_candidates[0].get("labels") or [])
        if bounded_candidates
        else []
    )
    ...
    quality = {
        ...
        "recent_high_signal_selected": "recent-high-signal-readout"
    ...
    pass_checks = {
        ...
        == "snn_language_readout_replay_priority.v1",
    ...
    return {
        "surface": "bounded_snn_readout_replay_priority_source_window_benchmark.v1",
        ...
        "retired_path_comparison": {
            "old_policy": "score_all_retained_readout_events_before_limit",
