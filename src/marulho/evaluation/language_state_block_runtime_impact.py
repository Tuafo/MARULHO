"""Measure LM forward impact from no-grad state-block sequence preallocation."""
...
_STATE_PREALLOC_ENV = "MARULHO_LANGUAGE_STATE_BLOCK_PREALLOCATE_NO_GRAD"
...
class StateBlockRuntimeImpactConfig:
...
def _model_config(
    config: StateBlockRuntimeImpactConfig,
...
def _run_arm(
    ...
    *,
    ...
    try:
        ...
        model.eval()
        with (
            _env_value(_STATE_PREALLOC_ENV, "1" if preallocate_no_grad else "0"),
        ...
        return {
            "surface": "marulho_language_state_block_runtime_arm.v1",
            "name": name,
            "success": True,
            "failure_reason": None,
            "preallocate_no_grad": bool(preallocate_no_grad),
            "state_block_preallocation_env": "1" if preallocate_no_grad else "0",
...
def _comparison(
    ...
    *,
    ...
    preallocated_logits: torch.Tensor | None,
) -> dict[str, Any]:
    baseline_success = bool(baseline.get("success"))
    preallocated_success = bool(preallocated.get("success"))
    baseline_tps = float(baseline.get("tokens_per_second", 0.0) or 0.0)
    preallocated_tps = float(preallocated.get("tokens_per_second", 0.0) or 0.0)
    speedup = (
        preallocated_tps / baseline_tps
        if baseline_success and preallocated_success and baseline_tps > 0.0
        else None
    )
    max_abs_error: float | None = None
    max_rel_error: float | None = None
    parity_passed = False
    if baseline_logits is not None and preallocated_logits is not None:
        diff = (preallocated_logits - baseline_logits).abs()
    ...
    preallocated_mode = str(
        preallocated.get("mixed_state_sequence_buffer_mode", "unknown")
    )
    baseline_mode = str(baseline.get("mixed_state_sequence_buffer_mode", "unknown"))
    if (
        baseline_success
        and preallocated_success
        and baseline_mode == "stacked_mixed_state_list"
        and preallocated_mode == "preallocated_no_grad_mixed_state_sequence"
    ):
        evidence_status = "measured_preallocated_vs_stacked_state_block_forward"
    elif baseline_success and preallocated_success:
        evidence_status = "measured_without_state_block_preallocation_switch"
    else:
        evidence_status = "runtime_impact_measurement_failed"
    return {
        "surface": "marulho_language_state_block_runtime_comparison.v1",
        "baseline_success": baseline_success,
        "preallocated_success": preallocated_success,
        "baseline_tokens_per_second": baseline_tps,
        "preallocated_tokens_per_second": preallocated_tps,
        "preallocated_vs_baseline_tokens_per_second_ratio": speedup,
        "baseline_buffer_mode": baseline_mode,
        "preallocated_buffer_mode": preallocated_mode,
...
def run_language_state_block_runtime_impact(
    *,
    output_path: str | Path,
    config: StateBlockRuntimeImpactConfig | None = None,
) -> dict[str, Any]:
    cfg = config or StateBlockRuntimeImpactConfig()
    ...
    baseline_report, baseline_logits = _run_arm(
        "stacked_mixed_state_list_baseline",
        preallocate_no_grad=False,
    ...
    preallocated_report, preallocated_logits = _run_arm(
        "preallocated_no_grad_mixed_state_sequence",
        preallocate_no_grad=True,
        base_state=base_state,
        batch=batch,
        config=cfg,
        tokenizer=tokenizer,
    ...
    comparison = _comparison(
        ...
        preallocated_logits=preallocated_logits,
    )
    report = {
        ...
        "split": split_report,
        "arms": {
            "stacked_mixed_state_list_baseline": baseline_report,
            "preallocated_no_grad_mixed_state_sequence": preallocated_report,
        },
        ...
        "review": {
            ...
            "selective_scan_fusion_claimed": False,
            "promotes_hot_path": False,
            "promotes_runtime_claim": False,
            "next_experiment": (
                "state-selective-scan fusion still needs a safe PLIF/scan design "
                "because spikes and leak depend on prior selective state"
            ),
        },
        "promotion_gate": {
            "runtime_impact_available": bool(
                comparison["baseline_success"] and comparison["preallocated_success"]
            ),
            "state_block_preallocation_used": bool(
                comparison["preallocated_buffer_mode"]
                == "preallocated_no_grad_mixed_state_sequence"
            ),
...
def main() -> int:
    ...
    report = run_language_state_block_runtime_impact(
    ...
    print(
        "wrote "
        f"{args.output} preallocated_tps="
        f"{comparison['preallocated_tokens_per_second']:.3f} "
        f"baseline_tps={comparison['baseline_tokens_per_second']:.3f} "
        f"ratio={comparison['preallocated_vs_baseline_tokens_per_second_ratio']}"
    )
    return 0
