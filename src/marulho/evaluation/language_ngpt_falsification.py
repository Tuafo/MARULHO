"""Recipe-separated falsification for the hyperspherical Transformer v6."""
...
SURFACE = "marulho_hyperspherical_transformer_falsification.v1"
ARTIFACT_KIND = "marulho_hyperspherical_transformer_falsification"
ARCHITECTURES = ("transformer", "normalized")
ARM_NAMES = (
...
class HypersphericalTransformerFalsificationConfig:
    ...
    standard_learning_rate: float = 3.0e-4
    standard_minimum_learning_rate_fraction: float = 0.10
    standard_warmup_fraction: float = 0.05
    standard_weight_decay: float = 0.10
    native_learning_rate: float = 1.5e-3
    native_minimum_learning_rate_fraction: float = 0.0
    native_warmup_fraction: float = 0.0
    native_weight_decay: float = 0.0
    ...
    baseline_width: int = 512
    baseline_layers: int = 4
    baseline_attention_heads: int = 8
    baseline_mlp_ratio: float = 4.0
    normalized_width: int = 448
    normalized_layers: int = 4
    normalized_attention_heads: int = 8
    normalized_hidden_width: int = 1936
    execution_backend: str = "eager"
    compile_loss_tolerance: float = 1.0e-3
    projection_norm_tolerance: float = 1.0e-5
...
def _architecture(name: str) -> str:
    return "normalized" if name.startswith("normalized_") else "transformer"


def _recipe(name: str) -> str:
    return "native" if name.endswith("_native") else "standard"
...
def _build_model(
    ...
    *,
    vocab_size: int,
    config: HypersphericalTransformerFalsificationConfig,
) -> MarulhoLanguageModel | MarulhoNormalizedLanguageModel:
    if architecture == "transformer":
        return MarulhoLanguageModel(
            LanguageModelConfig(
                vocab_size=int(vocab_size),
                embedding_dim=config.baseline_width,
                state_dim=config.baseline_width,
                state_layers=config.baseline_layers,
                attention_heads=config.baseline_attention_heads,
                transformer_context_length=config.sequence_length,
                transformer_mlp_ratio=config.baseline_mlp_ratio,
                tie_embeddings=True,
                active_language_path="marulho_transformer_v6_control",
            )
        )
    if architecture == "normalized":
        return MarulhoNormalizedLanguageModel(
            NormalizedTransformerConfig(
                vocab_size=int(vocab_size),
                width=config.normalized_width,
                layers=config.normalized_layers,
                attention_heads=config.normalized_attention_heads,
                hidden_width=config.normalized_hidden_width,
...
def _training_config(
    ...
    if recipe == "standard":
        ...
        minimum_fraction = config.standard_minimum_learning_rate_fraction
        warmup_fraction = config.standard_warmup_fraction
        weight_decay = config.standard_weight_decay
    elif recipe == "native":
        learning_rate = config.native_learning_rate
        minimum_fraction = config.native_minimum_learning_rate_fraction
        warmup_fraction = config.native_warmup_fraction
        weight_decay = config.native_weight_decay
    else:
        raise ValueError(f"Unknown training recipe: {recipe}")
    return LanguageTrainingExperimentConfig(
        learning_rate=learning_rate,
        minimum_learning_rate_fraction=minimum_fraction,
...
def _prepare_projection_backend(
    model: MarulhoLanguageModel | MarulhoNormalizedLanguageModel,
    *,
    config: HypersphericalTransformerFalsificationConfig,
) -> tuple[Callable[[], None], dict[str, Any]]:
    if not isinstance(model, MarulhoNormalizedLanguageModel):
        return (lambda: None), {
            "required": False,
            ...
            "post_compile_evidence": None,
        }
    initial = model.hyperspherical_weight_evidence()
    if config.execution_backend == "eager":
        return model.post_optimizer_step, {
            "required": True,
            "backend": "pytorch_eager",
            "compile_seconds": 0.0,
            "compile_peak_cuda_memory_bytes": 0,
            "initial_evidence": initial,
            "post_compile_evidence": initial,
        }
    compiled = torch.compile(
        model.project_hyperspherical_weights_,
        backend="inductor",
        fullgraph=True,
        dynamic=False,
    )
    ...
    if model.device.type == "cuda":
        torch.cuda.synchronize(model.device)
    elapsed = time.perf_counter() - started
    observed = model.hyperspherical_weight_evidence()
    if (
        float(observed["maximum_unit_norm_error"])
        > float(config.projection_norm_tolerance)
    ):
        raise RuntimeError(
            "Compiled hyperspherical projection exceeded norm tolerance: "
            f"{observed['maximum_unit_norm_error']:.8f} > "
            f"{config.projection_norm_tolerance:.8f}"
        )
    return compiled, {
        "required": True,
        "backend": "torch_compile_inductor_fullgraph",
        "compile_seconds": elapsed,
        "compile_peak_cuda_memory_bytes": (
            int(torch.cuda.max_memory_allocated(model.device))
            if model.device.type == "cuda"
            else 0
        ),
        "initial_evidence": initial,
        "post_compile_evidence": observed,
    }


def _normalized_scalar_evidence(
    model: MarulhoLanguageModel | MarulhoNormalizedLanguageModel,
) -> dict[str, Any] | None:
    if not isinstance(model, MarulhoNormalizedLanguageModel):
        return None
    ...
    for block in model.state_block.layers:
        alpha_attention.append(
            (
                block.attention_alpha
                * (block.alpha_init / block.base_scale)
            ).detach()
        )
        alpha_mlp.append(
            (
        ...
        suv.append(
            (block.suv * math.sqrt(float(block.width))).detach()
        )
    return {
        "logit_scale_mean": float(
            (model.logit_scale / model.base_scale).mean().detach().cpu()
        ),
        "attention_alpha_mean": float(
            torch.cat(alpha_attention).mean().float().cpu()
        ),
        "mlp_alpha_mean": float(torch.cat(alpha_mlp).mean().float().cpu()),
        "qk_scale_mean": float(torch.cat(sqk).mean().float().cpu()),
        "uv_scale_mean": float(torch.cat(suv).mean().float().cpu()),
    }
...
def _run_arm(
    ...
    *,
    ...
    projection_step: Callable[[], None],
    shared_execution: Mapping[str, Any],
    projection_execution: Mapping[str, Any],
    ...
    recipe = _recipe(name)
    training_config = _training_config(recipe, config)
    ...
    for step in range(total_steps):
        ...
        if (step + 1) % max(1, total_steps // 10) == 0:
            print(
                f"[hyperspherical-v6] {name} {step + 1}/{total_steps}",
                flush=True,
            )
    if device.type == "cuda":
        torch.cuda.synchronize(device)
    ...
    projection_evidence = (
        model.hyperspherical_weight_evidence()
        if isinstance(model, MarulhoNormalizedLanguageModel)
        else None
    )
    if (
        projection_evidence is not None
        and float(projection_evidence["maximum_unit_norm_error"])
        > float(config.projection_norm_tolerance)
    ):
        raise RuntimeError("Final hyperspherical projection audit failed")
    loss_trace = [
        {
            "step": step,
    ...
    return {
        ...
        "training": {
            ...
            "projection_cost_included": isinstance(
                model,
                MarulhoNormalizedLanguageModel,
            ),
            "learning_rate_peak": float(training_config.learning_rate),
        ...
        "execution": {
            **dict(shared_execution),
            "loss_graph_shared_between_architecture_recipes": True,
            "projection": dict(projection_execution),
        },
        "heldout": heldout,
        "relation": relation,
        "runtime_state_bytes_for_training_batch": state_bytes,
        "telemetry": sample_output["telemetry"],
        "hyperspherical_projection": projection_evidence,
        "normalized_scalars": _normalized_scalar_evidence(model),
    }


def hyperspherical_transformer_decision(
    ...
    *,
    ...
    normalized_standard_win = qualified(
        "normalized_standard",
        "transformer_standard",
    )
    normalized_native_same_recipe_win = qualified(
        "normalized_native",
        "transformer_native",
    )
    normalized_native_frozen_baseline_win = qualified(
        "normalized_native",
        "transformer_standard",
    )
    transformer_recipe_win = qualified(
        "transformer_native",
        "transformer_standard",
    )
    if normalized_standard_win or (
        normalized_native_same_recipe_win
        and normalized_native_frozen_baseline_win
    ):
        return "scale_v6_hyperspherical_transformer_to_64m"
    if transformer_recipe_win:
        return "adopt_native_recipe_on_transformer_retire_v6_architecture"
    normalized_best_free = max(
        free("normalized_standard"),
        free("normalized_native"),
    )
    transformer_best_free = max(
        free("transformer_standard"),
        free("transformer_native"),
    )
    normalized_best_loss = min(
        loss("normalized_standard"),
        loss("normalized_native"),
    )
    transformer_best_loss = min(
        loss("transformer_standard"),
        loss("transformer_native"),
    )
    if (
        normalized_best_free >= transformer_best_free + 0.02
        and normalized_best_loss > transformer_best_loss - 0.005
    ):
        return "redesign_v6_behavior_signal_without_loss_gain"
    return "retire_v6_no_architecture_quality_gain"
...
def _assemble_report(
    *,
    ...
    decision = hyperspherical_transformer_decision(
        arms,
    )
    counts = {row["name"]: int(row["parameters"]) for row in arms}
    transformer_count = next(
        (
            int(row["parameters"])
            for row in arms
    ...
    normalized_count = next(
        (
            int(row["parameters"])
            for row in arms
            if row["architecture"] == "normalized"
        ),
        None,
    )
    parameter_delta = (
        (normalized_count - transformer_count) / transformer_count
        if transformer_count and normalized_count
        else None
    )
    compile_seconds = sum(
    ...
    return {
        ...
        "recipes": {
            "standard": {
                ...
                "minimum_learning_rate_fraction": (
                    config.standard_minimum_learning_rate_fraction
                ),
                "warmup_fraction": config.standard_warmup_fraction,
                "weight_decay": config.standard_weight_decay,
            },
            "native": {
                "learning_rate": config.native_learning_rate,
                "minimum_learning_rate_fraction": (
                    config.native_minimum_learning_rate_fraction
                ),
                "warmup_fraction": config.native_warmup_fraction,
                "weight_decay": config.native_weight_decay,
                "source": "official_nGPT_public_recipe",
            },
        },
        "parameter_match": {
            "counts": counts,
            "normalized_minus_transformer_fraction": parameter_delta,
        ...
        "compile_reuse": {
            ...
            "loss_graph_compile_count_actual": actual_loss_compile_count,
        ...
        "decision_contract": {
            ...
            "normalized_standard_compared_with_transformer_standard": True,
            "normalized_native_compared_with_transformer_native": True,
            "normalized_native_must_also_beat_frozen_standard_baseline": True,
            "recipe_only_gain_does_not_promote_architecture": True,
...
def run_hyperspherical_transformer_falsification(
    *,
    ...
    config: HypersphericalTransformerFalsificationConfig = (
        HypersphericalTransformerFalsificationConfig()
    ),
    device: str = "auto",
    ...
    if config.projection_norm_tolerance <= 0.0:
        raise ValueError("projection_norm_tolerance must be positive")
    resolved = _resolve_device(device)
    if config.execution_backend == "inductor" and resolved.type != "cuda":
        raise ValueError("Inductor v6 execution is admitted only for CUDA runs")
    ...
    if not requested_arms or any(name not in ARM_NAMES for name in requested_arms):
        raise ValueError("arm_names must contain valid unique v6 arm names")
    ...
    if baseline_report_path is not None:
        ...
        if reused.get("surface") != SURFACE:
            raise ValueError("Baseline report surface does not match v6 runner")
    ...
    try:
        for architecture in ARCHITECTURES:
            ...
            if not group_arms:
                continue
            print(
                f"[hyperspherical-v6] preparing shared {architecture} graph",
                flush=True,
            )
            torch.manual_seed(config.model_seed)
            if torch.cuda.is_available():
            ...
            representative_config = _training_config("standard", config)
            warm_batch = LanguageBatch(
                staged.input_ids[0],
                staged.target_ids[0],
            )
            model.train()
            training_loss, loss_execution = _prepare_language_loss_backend(
                model,
                warm_batch,
                representative_config,
            )
            projection_step, projection_execution = (
                _prepare_projection_backend(model, config=config)
            )
            compile_seconds = float(loss_execution["compile_seconds"]) + float(
                projection_execution["compile_seconds"]
            )
            model.eval()
            ...
            for name in group_arms:
                print(f"[hyperspherical-v6] starting {name}", flush=True)
                row = _run_arm(
                    name,
                    model=model,
                    ...
                    shared_execution=loss_execution,
                    projection_execution=projection_execution,
                ...
                print(
                    f"[hyperspherical-v6] completed {name}: loss "
                ...
                write_json_report_with_readme(
                    output,
                    partial,
                    title="MARULHO Hyperspherical Transformer v6 Falsification",
                )
            del projection_step, training_loss, model
            gc.collect()
            if resolved.type == "cuda":
    ...
    print(f"[hyperspherical-v6] decision {report['decision']}", flush=True)
    return report
...
def main() -> int:
    ...
    run_hyperspherical_transformer_falsification(
