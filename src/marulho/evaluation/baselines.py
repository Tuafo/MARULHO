"""Baseline models for calibrating MARULHO evaluation metrics (§8.1).
...
All baselines are trained on the same text corpus that MARULHO uses,
...
class OnlineSOM:
    ...
    Same dimension and prototype count as MARULHO, but no SNN dynamics,
...
class BaselineResults:
    ...
    def _calibrated_target(self) -> dict[str, Any]:
        """Calibrate MARULHO target based on fastText score (§8.1)."""
        ft_score = self.fasttext_probe.total_accuracy
        return {
            "fasttext_score": ft_score,
            "marulho_text_only_target": ft_score,
            "marulho_multimodal_target": ft_score + 0.05,
            "note": "MARULHO multimodal should exceed fastText + 0.05; "
...
def run_all_baselines(
    ...
    Args:
        corpus: Text corpus to train on (same as MARULHO training corpus).
        input_dim: Dimensionality for SOM vectors.
        n_prototypes: Number of SOM prototypes (matches MARULHO column count).
