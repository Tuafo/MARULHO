...
class MarulhoConfig:
    ...
    def __post_init__(self) -> None:
        ...
        if self.candidate_memory_pressure_filter_start_tokens < 0:
            raise ValueError("candidate_memory_pressure_filter_start_tokens must be non-negative")
        if self.candidate_memory_pressure_backfill_factor <= 0:
        ...
        if self.binding_mode not in ("dense", "spatial", "hypercube"):
            raise ValueError("binding_mode must be 'dense', 'spatial', or 'hypercube'")
        ...
        if self.routing_index_mode != "torch_topk":
            raise ValueError(
                "routing_index_mode must be torch_topk; auto, faiss_hnsw, and exact_cosine are retired"
            )
    ...
    def device_report(self) -> dict[str, object]:
        ...
        report: dict[str, object] = {
            ...
            "env_device": os.environ.get("MARULHO_DEVICE"),
