...
from marulho.evaluation.grounding_probe import evaluate_grounding_probe
from marulho.semantics.geometric_curiosity import GeometricCuriosityController
from marulho.training.runner_utils import set_seed
from marulho.training.query_runner import feed_text
...
def _make_config_for_stage(
    stage: int, base_config: MarulhoConfig | None = None
) -> MarulhoConfig:
    ...
    """
    import copy
    cfg = copy.deepcopy(base_config) if base_config is not None else MarulhoConfig()
...
def _make_vector_fn(
    trainer: MarulhoTrainer, encoder: BaseEncoder, cfg: MarulhoConfig
):
...
def _train_on_real_digits(
    ...
    Args:
        trainer: MARULHO trainer instance.
...
def run_stage_1(
    config: MarulhoConfig | None = None,
    n_tokens: int = 5000,
    seed: int = 7,
...
def run_baseline_calibration(
    ...
    """
    from marulho.evaluation.baselines import run_all_baselines
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="MARULHO training — developmental validation or production training",
    )
    ...
    if args.dataset_name:
        ...
        from marulho.training.train_runner import train
