...
from marulho.data.corpus_loader import huggingface_token_from_env, project_dataset_columns
...
def _download_binary_asset(url: str, *, timeout_seconds: float = 20.0) -> bytes:
    request = Request(
        str(url),
        headers={
            "User-Agent": "MARULHO/1.0 (+https://github.com/) hf-asset-loader",
            "Accept": "*/*",
        },
    )
...
def _ensure_s1_recaption_index_loading(source: str, year_prefix: str) -> Event | None:
    ...
    thread = Thread(target=_runner, name=f"marulho-s1-recaption-{key[1]}", daemon=True)
    thread.start()
    return ready
