...
def _ensure_isolated_output_dir(output_dir: str | Path, *, repo_root: Path | None = None) -> Path:
    ...
    forbidden_roots = [root / "src", root / "checkpoints", root / "reports" / "service", root / "HECSN_UI"]
...
def run_isolated_replay_adapter_experiment(
    *,
    ...
    command_record = list(command or ["python", "-m", "hecsn.training.replay_adapter_experiment"])
    manifest = {
