...
from hecsn.config.presets import autonomy_preset_names, get_autonomy_preset
from hecsn.data import expand_source_bank_specs
...
def concept_frontier_metrics(trainer: HECSNTrainer, bank: SourceBank) -> tuple[float, float, float]:
...
def probe_diagnostics(trainer: HECSNTrainer, patterns: list[torch.Tensor]) -> ProbeDiagnostics:
    if not patterns:
        return {
...
def make_config(args: dict[str, Any]) -> HECSNConfig:
    return HECSNConfig(
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(description="Run HECSN autonomy benchmark with gap-driven source selection")
