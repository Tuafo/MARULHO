...
**Status:** Executable Research Scaffold — maintained path covers mechanism validation, memory consolidation, contextual routing, checkpoint/query tooling, and HF active acquisition under projected lookahead; `autonomy_hf_smoke`, `autonomy_hf_baseline`, and `memory_consolidation_hf_scale_robust` are green on the current tree, while HF scout plus open-web acquisition remain exploratory
**Stack:** PyTorch 2.1+ · CUDA · 2:4 Structured Sparsity · CSR Sparse Tensors · FAISS (CPU)

**Executable status (2026-04-03):** The maintained smoke gate now passes on the mechanism-validation benchmark (`silhouette ~= 0.675`, `DBI ~= 0.304`, `trained_eval_recon_error 0.0619 < random_assignment 0.0907`). The maintained runtime now also exposes a maintained local plasticity path (`plasticity_mode = local_stdp`) with log-STDP-style eligibility traces, iSTDP-style inhibitory balancing, synaptic scaling, and plastic latent projections, plus a multiscale recurrent context circuit with fast/medium/slow traces and a short-term-plasticity binding circuit with PV-like inhibition over column assemblies. The memory-consolidation path now uses an explicit phenomenological tag/PRP replay-consolidation stack: the slow buffer tracks decaying capture tags, local and shared PRP state, consolidation level, replay spacing, and replay counts, and sleep replay recruits PRP before converting capture pressure into increasing consolidation state. The shared semantic concept layer is no longer a pure cluster-memory proxy: `semantics/concepts.py` now maintains an online slow-feature abstraction over retrieved routing signatures, so concept summaries emphasize temporally stable invariants rather than only nearest-memory grouping. The contextual-routing path continues to include explicit B3 `bank` lexical-bundle probes on the maintained `order_weighted_ascii` + contextual-routing + binding-conjunction runtime (`bank_polysemy_accuracy ~= 0.875`, `signature_margin ~= 0.00028`); on the current maintained path, winner-sequence separation is no longer the honest regression point, so signature-level family separation is the maintained contextual-disambiguation metric. Autonomy remains executable, and the maintained controller now uses explicit probe/frontier gap plans with retrieval queries and follow-up questions: probe diagnostics include semantic grounding pressure and a semantic action score, while acquisition shortlists score candidate banks against the shared frontier plan in addition to projected lookahead over candidate gaps. The maintained acquisition path is no longer limited to fixed candidate lists inside presets: source specs can now expand semantic registries of remote HF entries against the current frontier plan before training begins. The same source-spec plumbing can also ingest open-web entries, but that branch remains exploratory rather than part of the maintained regression target. The local service/UI now exposes checkpoint-backed query/respond flows, benchmark report inspection, learned concept-memory summaries with uncertainty/drift, response-side concept grounding, and acquisition preset/policy control with optional manual overrides. A maintained representation smoke benchmark now compares `order_weighted_ascii`, `unigram_ascii`, and `hashed_ngram` against an `OnlineKMeans` baseline on 4K/1K WikiText windows; on that slice, all three HECSN competitive-only runs beat the baseline on silhouette, while `hashed_ngram` is numerically strongest and `order_weighted_ascii` remains the maintained runtime default. Fresh maintained reruns on the current environment now keep the core smoke/baseline slices green overall: `autonomy_hf_smoke` and `autonomy_hf_baseline` both pass again on the explicit `news/wiki/reviews` source bank under the scale-aware autonomy gate (`reports/phase3_autonomy_smoke_after/summary.json`, active mean/max gap `~= 1.57e-5/1.57e-5` vs round-robin `2.55e-5/2.55e-5`; `reports/phase3_autonomy_baseline_after/summary.json`, active mean/max gap `~= 9.23e-7/9.29e-7` vs round-robin `1.37e-6/1.39e-6`). The maintained `memory_consolidation_hf_scale_robust` preset also remains green after aligning capture-tag decay with the documented functional-minute time base (`reports/phase2_scale_robust_after/summary.json`, `task_a_relative_degradation_after_consolidation ~= 0.0103`, `task_a_overlap_after_consolidation ~= 0.99999`), and a fresh corrected `50K/50K` no-final-consolidation rerun now passes again as well (`reports/phase2_scale_stress_after_metric_fix/summary.json`, `task_a_relative_degradation_after_consolidation = 0.0`, `task_a_overlap_after_consolidation ~= 0.99999`), confirming the immediately prior red result was numerical rather than behavioral. The maintained HF multi-slot acquisition benchmark now also passes again after unifying offline/live acquisition execution and switching active allocation to RNG-stable projected frontier lookahead (`reports/refactor_autonomy_acquisition_hf_allocation_rng_20260401/summary.json`, active mean/max candidate gap `~= 0.137/0.148` vs round-robin `0.244/0.275`). Under that corrected executor the maintained HF acquisition surface is projected active allocation; the visible HF scout preset is `autonomy_acquisition_hf_scout_exploratory`. With `scout_top_k = 1` it matches active rather than beating it (`reports/refactor_autonomy_acquisition_hf_scout_projected_20260401/summary.json`), and small `top_k = 2` sweeps stay below the active baseline, so HF scout remains exploratory. Fresh confirmed open-web reruns on the current tree no longer preserve even the active-allocation win: both active and scout-and-commit now finish worse than round-robin on the curated Wikipedia frontier (`reports/refactor_autonomy_acquisition_open_web_scout_projected_20260402/summary.json`, confirmed by `reports/refactor_autonomy_acquisition_open_web_scout_confirm_20260402/summary.json`), so the full open-web acquisition path should currently be treated as exploratory.
...
```
╔═══════════════════════════════════════════════════════════════════╗
║ [FUTURE: Abstraction Layer — Stage 3]                              ║
║ • Planned: Slow Feature Analysis (Wiskott & Sejnowski 2002)       ║
║ • Not part of current specification                                ║
╠═══════════════════════════════════════════════════════════════════╣
...
Instead of a single global modulator, the full HECSN design computes **precision-weighted prediction errors per layer**. The maintained executable scaffold currently exposes this mechanism on the competitive path and derives dopamine / acetylcholine / norepinephrine proxies from recent error history; context- and binding-specific error paths are still benchmarked behaviorally rather than gated by separate live plasticity channels:
...
├── service/
│   ├── __init__.py
│   ├── api.py             # FastAPI app over checkpoint/query/respond/acquisition/report flows
│   ├── concepts.py        # Learned concept memory over retrieved routing traces
│   ├── manager.py         # Serialized mutable runtime wrapper + persisted concept state
│   ├── schemas.py         # Request/response models for the local service
│   ├── server.py          # Uvicorn launcher for the local service
│   └── benchmark_reports.py # Archived benchmark loader for /reports/benchmarks
├── training/
│   ├── __init__.py
...
│   └── src/
│       ├── App.jsx        # Live chat, concept/acquisition controls, telemetry, checkpoint controls
│       ├── main.jsx       # Frontend bootstrap
│       └── index.css      # Motion + visual system for the local operator UI
└── monitoring/

Implementation note:
- The executable scaffold currently available in this repository covers config, encoder, competitive routing, surprise modulation, memory drift tracking, memory-consolidation benchmarking, a runnable contextual-routing path, a runnable autonomy benchmark for concept-frontier-aware source selection, and a runnable acquisition benchmark for selecting which unseen source bank to ingest next under `src/hecsn/`, with maintained HF source banks and exploratory curated open-web URL source banks routed through the unified corpus loader plus semantic expansion over remote source registries.
- The executable scaffold now also supports checkpoint save/load plus a direct raw-text query path: `checkpointing.py` persists the current network state; the maintained `mechanism_validation_runner.py`, `memory_consolidation_runner.py`, `contextual_routing_runner.py`, `hierarchical_scale_runner.py`, `autonomy_runner.py`, and `autonomy_acquisition_runner.py` can each emit queryable checkpoints via `--checkpoint-out`; and `query_runner.py` can load that state, feed additional unlabeled text online, retrieve the nearest stored memory windows and candidate columns for a new text query, emit an initial native assembly-to-text decode summary by stitching overlapping remembered windows, or compare the same query under two different context primes.
- The repository now also includes a local interactive surface: `interaction/responder.py` implements a strict-evidence response mode over retrieved memories plus a guarded native-decode response mode when the stitched continuation is confident enough; `service/api.py` + `service/manager.py` wrap checkpoint-backed HECSN state behind serialized FastAPI endpoints for status, feed, query, respond, checkpoint save/restore, acquisition preset/run, archived benchmark summaries, persisted recent turn traces under `reports/service/traces/`, and a live `/stream/status` telemetry feed.
...
- `web/` now provides a React/Vite operator interface for chat, evidence inspection, checkpoint control, live telemetry, benchmark plots, concept-summary/concept-grounding inspection, and acquisition control through preset/policy selection plus optional manual overrides; the concept panels now surface learned-cluster support, observation count, uncertainty, and drift.
...
- Scope note on the autonomy benchmark: this is still a source-selection proxy for "detects its own knowledge gaps, and actively seeks information to fill them". It now validates closed-loop gap measurement, explicit probe/frontier question generation, semantic gap planning over retrieved memory traces, and adaptive corpus choice in the executable scaffold across maintained HF datasets, while curated open-web URL banks use the same machinery only in exploratory mode. The maintained runner can now expand remote HF semantic registries against the frontier before forming the candidate bank. It still does not implement unconstrained web search or broader open retrieval beyond the configured remote registries. The richer ambiguity/switch diagnostics are now logged, projected frontier lookahead is the maintained HF acquisition path, and both HF scout plus all current open-web acquisition presets remain exploratory until they reproduce wins over their baselines on fresh reruns.
