...
- offline replay training gate command: `hecsn.evaluation.replay_training_gate` evaluates a saved bundle JSON without loading a manager, training, mutating memory, posting feedback, executing actions, starting sleep, or making external calls; the current saved report `reports/replay_training_gate_report.json` passed **7/7** checks and still reports `eligible_for_training=false`.
- delayed-consequence branch-match stabilization: exact stored supportive/adverse branch query examples now deterministically satisfy the match gate, removing a split/recovery threshold flake while leaving unrelated fuzzy matching governed by the existing threshold.
...
- The offline replay gate can now satisfy the regression/check portion for a specific saved bundle, but it intentionally stops at `passed_pending_operator_training_approval`; the concrete next boundary is a signed/recorded approval artifact for a specific `bundle_hash` and gate report, not adapter training.
